...
const { loadBCP, applyBCPUpdates, calculateOnboardingScore, renderBCPasMarkdown } = require('./lib/lunar/bcpManager');
const { buildLunarSystemPrompt } = require('./lib/lunar/prompt');
const { parseAndExecuteActions } = require('./lib/agents/parseActions');
const { buildAgentSystemPrompt } = require('./lib/ai/prompt');
...
async function testAll() {
  ...
  const mockAgent = { name: 'Aria', id: 'aria-1', acronym: 'CMO' };
  const mockCompany = { company_name: 'Nexonic AI', industry: 'SaaS', mission: 'Automate C-Suite' };
