  ...
  if (agentName === 'Aria') {
    if (connectedServices.includes('linkedin')) {
      tools.linkedin_post = tool({
        ...
        execute: async ({ content }) => {
          try { return await executeViaComposio(orgId, 'linkedin', 'linkedin_post', { content }) }
          catch (err: any) { return { error: `LinkedIn unavailable: ${err.message}` } }
        },
      })
    }
    if (connectedServices.includes('twitter')) {
      tools.twitter_post = tool({
        ...
        execute: async ({ text }) => {
          try { return await executeViaComposio(orgId, 'twitter', 'twitter_post', { text }) }
          catch (err: any) { return { error: `Twitter unavailable: ${err.message}` } }
        },
      })
    }
  }

  if (agentName === 'Rex' && connectedServices.includes('hubspot')) {
    tools.hubspot_create_deal = tool({
      ...
      execute: async (params) => {
        try { return await executeViaComposio(orgId, 'hubspot', 'hubspot_create_deal', params) }
        catch (err: any) { return { error: `HubSpot unavailable: ${err.message}` } }
      },
    })
  }

  if (agentName === 'Ghost' && connectedServices.includes('github')) {
    tools.github_create_pr = tool({
      description: 'Create a GitHub Pull Request.',
      parameters: z.object({ repo: z.string(), title: z.string(), body: z.string(), branch: z.string() }),
      execute: async (params) => {
        try { return await executeViaComposio(orgId, 'github', 'github_create_pr', params) }
        catch (err: any) { return { error: `GitHub unavailable: ${err.message}` } }
      },
    })
  }
